...
import * as clubApi from "./clubApi";
...
export default function Link() {
  ...
  const columns: any = [
    ...
    },
    {
      ...
      key: "status",
      // eslint-disable-next-line react/display-name
      render: (_name: string, record: any) => (record?.status ? "Yes" : "No"),
    },
    {
      title: "Action",
      key: "action",
  ...
  const onFinish = async (values: any) => {
    try {
      ...
      await clubApi.updateItem(item?._id, values);
  ...
  const handleEdit = async (id: any) => {
    ...
    try {
      setloading(true);
      const item: any = await clubApi.getItem(id);
      form.setFieldsValue({
  ...
  const getItems = useCallback(
    async (params?: any) => {
      ...
      try {
        setloading(true);
        const response: any = await clubApi.getItems({ ...query });
    ...
    <Card title="Club List">
      ...
      >
        <Spin spinning={loading}>
          ...
          <Descriptions column={2} title="Club Info">
